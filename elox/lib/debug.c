...
static int interfaceInstruction(RunCtx *runCtx, const char *name, Chunk *chunk, int offset) {
	uint16_t nameConstant;
	memcpy(&nameConstant, &chunk->code[offset + 1], sizeof(uint16_t));
	uint16_t numMethods;
	memcpy(&numMethods, &chunk->code[offset + 3], sizeof(uint16_t));
	...
	eloxPrintf(runCtx, ELOX_IO_DEBUG, ") %5d\n", numMethods);
	for (int i = 0; i < numMethods; i++) {
		uint16_t methodName;
		memcpy(&methodName, &chunk->code[offset + 5 + 5 * i], sizeof(uint16_t));
		uint8_t arity = chunk->code[offset + 5 + 5 * i + 2];
		uint8_t hasVarargs = chunk->code[offset + 5 + 5 * i + 3];
		ELOX_WRITE(runCtx, ELOX_IO_DEBUG, "    ");
		printValue(runCtx, ELOX_IO_DEBUG, chunk->constants.values[methodName]);
		eloxPrintf(runCtx, ELOX_IO_DEBUG, " %u %u \n", arity, hasVarargs);
	}
	return offset + 5 + 4 * numMethods;
}
...
int disassembleInstruction(RunCtx *runCtx, Chunk *chunk, int offset) {
	...
	switch (instruction) {
		...
		case OP_INTERFACE:
			return interfaceInstruction(runCtx, "INTERFACE", chunk, offset);
		case OP_CLASS:
