			...
			for (int im = 0; im < intf->methods.capacity; im++) {
				...
				if (methodName != NULL) {
					...
					if (propInfo.type != ELOX_PROP_NONE) {
						...
						bool pushed = valueArrayPush(runCtx, &clazz->classData, entry->value);
						if (ELOX_UNLIKELY(!pushed)) {
							ELOX_THROW_RET_VAL(error, OOM(runCtx), ptr - ip);
						}
						uint32_t methodIndex = clazz->classData.count - 1;
						propTableSet(runCtx, &clazz->props, methodName,
									 (PropInfo){methodIndex, ELOX_PROP_METHOD}, error);
						if (ELOX_UNLIKELY(error->raised))
							return (ptr - ip);
					}
				}
...
static void defineMethod(RunCtx *runCtx, ObjString *name, EloxError *error) {
	...
	if ((name == clazz->name) || (name == vm->builtins.anonInitString))
		...
		if (methodExists) {
			...
			bool added = valueArrayPush(runCtx, &clazz->classData, OBJ_VAL(method));
...
static unsigned int addAbstractMethod(RunCtx *runCtx, CallFrame *frame, EloxError *error) {
	...
	switch (parent->type) {
		case OBJ_INTERFACE:
			...
			break;
		case OBJ_CLASS: {
			klazz = (ObjClass *)parent;
			PropInfo propInfo = propTableGetAny(&klazz->props, methodName);
			if (propInfo.type != ELOX_PROP_NONE) {
				if (ELOX_UNLIKELY(propInfo.type != ELOX_PROP_METHOD))
					ELOX_THROW_RET_VAL(error, RTERR(runCtx, "Method %.*s shadows existing property",
									   ...
									   ptr - ip);
				methodExists = true;
				existingMethod = klazz->classData.values[propInfo.index];
			}
			break;
		}
		default:
	...
	if (methodExists) {
		...
		switch (parent->type) {
			case OBJ_INTERFACE:
				...
				break;
			case OBJ_CLASS: {
				bool pushed = valueArrayPush(runCtx, &klazz->classData, OBJ_VAL(methodDesc));
				if (ELOX_UNLIKELY(!pushed))
					ELOX_THROW_RET_VAL(error, OOM(runCtx), ptr - ip);
				uint32_t methodIndex = klazz->classData.count - 1;
				// no need to check for now, we return after this anyway
				propTableSet(runCtx, &klazz->props, methodName,
							 (PropInfo){methodIndex, ELOX_PROP_METHOD}, error);
				break;
			}
			default:
...
static void defineField(RunCtx *runCtx, ObjString *name, EloxError *error) {
	...
	bool isNewField = propTableSet(runCtx, &clazz->props, name,
								   (PropInfo){index, ELOX_PROP_FIELD}, error);
	if (isNewField)
		clazz->numFields++;
}
...
static void defineStatic(RunCtx *runCtx, ObjString *name, EloxError *error) {
	...
	if (propInfo.type == ELOX_PROP_NONE) {
		bool pushed = valueArrayPush(runCtx, &clazz->classData, peek(fiber, 0));
		ELOX_CHECK_THROW_RET((pushed), error, OOM(runCtx));
		uint32_t staticIndex = clazz->classData.count - 1;
		propTableSet(runCtx, &clazz->props, name,
					 (PropInfo){staticIndex, ELOX_PROP_STATIC}, error);
		if (ELOX_UNLIKELY(error->raised))
			return;
	} else {
...
bool setInstanceField(ObjInstance *instance, ObjString *name, Value value) {
	...
	PropInfo valueInfo = propTableGet(&clazz->props, name, ELOX_PROP_FIELD);
	if (valueInfo.type != ELOX_PROP_NONE) {
		instance->fields.values[valueInfo.index] = value;
		return false;
	}
	return true;
}
	...
	do {
		...
		switch (propInfo.type) {
			...
			case ELOX_PROP_FIELD: {
				return callValue(runCtx, instance->fields.values[propInfo.index], argCount).result;
			}
			case ELOX_PROP_STATIC:
				return callValue(runCtx, clazz->classData.values[propInfo.index],
								 argCount).result;
			case ELOX_PROP_NONE:
				ELOX_UNREACHABLE();
				return false;
...
static bool invoke(RunCtx *runCtx, ObjString *name, int argCount) {
	...
	PropInfo methodInfo = propTableGet(&clazz->props, name, ELOX_PROP_METHOD);
...
Value toString(RunCtx *runCtx, Value value, EloxError *error) {
	...
	PropInfo methodInfo =
		propTableGet(&clazz->props, vm->builtins.biObject.toStringStr, ELOX_PROP_METHOD);
	if (ELOX_UNLIKELY(methodInfo.type == ELOX_PROP_NONE))
...
static void resolveMember(RunCtx *runCtx, ObjClass *clazz, uint8_t slotType,
	...
	if (super) {
		...
		PropInfo propInfo = propTableGet(&superClass->props, propName, ELOX_PROP_METHOD);
		...
		clazz->memberRefs[slot] = (MemberRef){
			...
			.isThis = false,
			.data.value = superClass->classData.values[propInfo.index]
		};
	} else {
		int32_t propIndex = -1;
		bool isField = false;
		...
		if (propType & MEMBER_FIELD) {
			PropInfo info = propTableGet(&clazz->props, propName, ELOX_PROP_FIELD);
		...
		if ((propIndex < 0) && (propType & MEMBER_METHOD)) {
			PropInfo propInfo = propTableGet(&clazz->props, propName, ELOX_PROP_METHOD);
			if (propInfo.type != ELOX_PROP_NONE)
		...
		if (isField) {
			...
			clazz->memberRefs[slot] = (MemberRef){
				.refType = REFTYPE_CLASS_MEMBER,
				.isThis = true,
				.data.value = clazz->classData.values[propIndex]
			};
		}
	}
}
