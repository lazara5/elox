...
/*
 * History
 * 2022-12-28 Adrian Lazar for eLox
 *
 * 	Adapted snprintf for use within eLox
 *
...
#define OUTCHAR(buffer, len, ch) \
do { \
	if (len + 1 < PRINTF_BUFFER_SIZE) { \
		buffer->data[len] = ch; \
		(len)++; \
	} else { \
...
static void fmtint(PrintfBuffer *, size_t *, INTMAX_T, int, int, int, int);
static void fmtflt(PrintfBuffer *, size_t *, LDOUBLE, int, int, int, int *);
static void printsep(PrintfBuffer *, size_t *);
	...
	while (ch != '\0')
		...
		case PRINT_S_CONV:
			...
			case 'i':
				...
				fmtint(buffer, &len, value, 10, width, precision, flags);
				break;
			case 'X':
				flags |= PRINT_F_UP;
				/* FALLTHROUGH */
			...
			case 'u':
				...
				fmtint(buffer, &len, value, base, width, precision, flags);
				break;
			case 'A':
			...
			case 'f':
				...
				fmtflt(buffer, &len, fvalue, width, precision, flags, &overflow);
				if (overflow)
					goto out;
				break;
			case 'c':
				cvalue = va_arg(args, int);
				OUTCHAR(buffer, len, cvalue);
				break;
			case 's':
				strvalue = va_arg(args, char *);
				fmtstr(buffer, &len, strvalue, width, precision, flags);
				break;
			case 'p':
				/*
				...
				if ((strvalue = va_arg(args, void *)) == NULL)
					 ...
					 * "0x0", SysV "0".
					 */
					fmtstr(buffer, &len, "(nil)", width, -1, flags);
				else {
					/*
					...
					flags |= PRINT_F_NUM;
					flags |= PRINT_F_UNSIGNED;
					fmtint(buffer, &len, (UINTPTR_T)strvalue, 16, width, precision, flags);
				}
				break;
			case 'n':
				switch (cflags) {
...
static void
fmtstr(PrintfBuffer *buffer, size_t *len, const char *value, int width,
...
static void
fmtint(PrintfBuffer *buffer, size_t *len, INTMAX_T value, int base, int width,
	   int precision, int flags) {
	UINTMAX_T uvalue;
	...
	if (sign != 0)	/* Sign. */
		OUTCHAR(buffer, *len, sign);
	if (hexprefix != 0) {	/* A "0x" or "0X" prefix. */
		OUTCHAR(buffer, *len, '0');
		OUTCHAR(buffer, *len, hexprefix);
	}
	...
	while (pos > 0) {	/* The actual digits. */
		pos--;
		OUTCHAR(buffer, *len, iconvert[pos]);
...
static void
fmtflt(PrintfBuffer *buffer, size_t *len, LDOUBLE fvalue, int width,
	...
	if (infnan != NULL) {
		...
		fmtstr(buffer, len, iconvert, width, ipos, flags);
		return;
	}
...
again:
	...
	while (ipos > 0) {	/* Integer part. */
		ipos--;
		OUTCHAR(buffer, *len, iconvert[ipos]);
	...
	while (fpos > omitcount) {	/* The remaining fractional part. */
		fpos--;
		OUTCHAR(buffer, *len, fconvert[fpos]);
	}
	while (epos > 0) {	/* Exponent. */
		epos--;
		OUTCHAR(buffer, *len, econvert[epos]);
	}
	while (padlen < 0) {	/* Trailing spaces. */
		OUTCHAR(buffer, *len, ' ');
		padlen++;
...
static void
printsep(PrintfBuffer *buffer, size_t *len) {
