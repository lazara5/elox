	...
	if (IS_OBJ(callee)) {
		switch (OBJ_TYPE(callee)) {
			...
			case OBJ_CLASS: {
				...
				vm.stackTop[-argCount - 1] = OBJ_VAL(newInstance(clazz));
				Value initializer;
				if (tableGet(&clazz->methods, vm.initString, &initializer)) {
					return callMethod(AS_OBJ(initializer), argCount);
				} else if (argCount != 0) {
...
static bool invokeFromClass(ObjClass *clazz, ObjString *name, int argCount) {
	...
	return callMethod(AS_OBJ(method), argCount);
}
...
static bool invoke(ObjString *name, int argCount) {
	...
	return invokeFromClass(instance->clazz, name, argCount);
}
...
#define BINARY_OP(valueType, op) \
	...
	for (;;) {
		...
		switch (instruction = READ_BYTE()) {
			...
			case OP_GET_UPVALUE: {
				uint8_t slot = READ_BYTE();
				push(*getFrameClosure(frame)->upvalues[slot]->location);
				break;
			}
			case OP_SET_UPVALUE: {
				uint8_t slot = READ_BYTE();
				*getFrameClosure(frame)->upvalues[slot]->location = peek(0);
				break;
			}
